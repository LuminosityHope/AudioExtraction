...
TEST_F(DemodulatorTest, DataCorrectlyAM)
{
	...
	EXPECT_EQ(demod[0], int(sqrt(5)));
	EXPECT_EQ(demod[1], int(sqrt(41)));
	EXPECT_EQ(demod[2], int(sqrt(130)));
	EXPECT_EQ(demod[3], int(sqrt(36)));

	DemodulatorAM<float> demodFloat;
	auto demodFl = demodFloat.demodulate(dataFloat);
	ASSERT_EQ(demodFl.size(), dataFloat.size());
	EXPECT_FLOAT_EQ(demodFl[0], (sqrt(1.25)));
	EXPECT_FLOAT_EQ(demodFl[1], (sqrt(22.1)));
	EXPECT_FLOAT_EQ(demodFl[2], (sqrt(67.86)));
}
